document.addEventListener("DOMContentLoaded", function () {
    ...
    // Password show hide
    function initPasswordToggle(spanId, containerClass, inputSelector, tooltipId = null) {
        const pwShowHide = document.getElementById(spanId);
        if (!pwShowHide) return;

        const container = pwShowHide.closest(containerClass);
        const passwordField = container.querySelector(inputSelector);
        const eyeOnIcon = pwShowHide.querySelector('.eye-on');
        const eyeOffIcon = pwShowHide.querySelector('.eye-off');
        const tooltip = tooltipId ? document.getElementById(tooltipId) : null;

        const syncIcons = () => {
            const isPwd = passwordField.type === 'password';
            eyeOnIcon.classList.toggle('active', isPwd);
            eyeOffIcon.classList.toggle('active', !isPwd);
        };
        ...
        pwShowHide.addEventListener('mousedown', (e) => {
            e.preventDefault();
            toggleNow();
        });

        pwShowHide.addEventListener('touchstart', (e) => {
            e.preventDefault();
            toggleNow();
        });

        pwShowHide.addEventListener('click', (e) => {
            e.preventDefault();
        });
    }

    // Field password in login form
    initPasswordToggle('pw-show-hide', '.password-container', 'input[name="password"]');

    // Field password in register form
    initPasswordToggle('pw-show-hide-register', '.password-container-register', 'input[name="password"]');

    // Field confirm password in register form
    initPasswordToggle('pw-show-hide-confirm', '.password-container-register', 'input[name="password_confirmation"]');

    // Field password in register teacher form
    initPasswordToggle('pw-show-hide-register-teacher', '.password-container-register-teacher', 'input[name="password"]');

    // Field confirm password in register teacher form
    initPasswordToggle('pw-show-hide-confirm-teacher', '.password-container-register-teacher', 'input[name="password_confirmation"]');
    
    // Field password in reset password form
    initPasswordToggle('pw-show-hide-reset-password', '.password-container-reset-password', 'input[name="password"]');

    // Field confirm password in reset password form
    initPasswordToggle('pw-show-hide-confirm-reset-password', '.password-container-reset-password', 'input[name="password_confirmation"]');

    // Field password in change password form
    initPasswordToggle('pw-show-hide-old-password-change-password', '.password-container-change-password', 'input[name="password_old"]');

    // Field password in change password form
    initPasswordToggle('pw-show-hide-change-password', '.password-container-change-password', 'input[name="password"]');

    // Field confirm password in change password form
    initPasswordToggle('pw-show-hide-confirm-change-password', '.password-container-change-password', 'input[name="password_confirmation"]');
